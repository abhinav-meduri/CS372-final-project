...
**Motivation and Architecture:** Traditional patent search is extremely time-consuming - patent examiners and inventors must manually search through millions of existing patents to find relevant prior art, a process that can take days or weeks even with keyword search tools. I built this system to automate patent search using machine learning, recognizing that patent search is fundamentally a similarity problem: given a query patent describing a new invention, I need to find existing patents that cover similar technical ground, rank them by relevance, and explain why they matter. This is what retrieval-augmented generation (RAG) systems excel at.

The system implements a four-stage pipeline: (1) **Embedding Generation** - PatentSBERTa transformer converts patent text into 768-dimensional vectors capturing semantic meaning, (2) **Hybrid Retrieval** - combines fast cosine similarity search over 200K local embeddings with LLM-powered keyword extraction that triggers Google Patents API searches for comprehensive coverage, (3) **Neural Reranking** - a custom PyTorch neural network with residual blocks scores the top-20 candidates using engineered features to produce calibrated similarity probabilities, and (4) **LLM Explanation** - Phi-3 generates natural language explanations analyzing technical overlaps between the query and retrieved prior art. The neural architecture uses [256, 128] hidden dimensions with dropout (0.3), L2 regularization, gradient clipping, and early stopping, achieving 91.20% accuracy and 97.01% ROC-AUC on citation-based evaluation.
