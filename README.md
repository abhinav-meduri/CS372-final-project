...
The PyTorch neural network classifier was trained on patent citation pairs extracted from USPTO data. Citations were used as positive examples because cited patents represent confirmed relevant prior art. Negative examples were generated by randomly sampling non-citing patent pairs from the corpus. This makes it a positive-unlabeled (PU) learning problem where the goal is to distinguish patents that are likely similar from those that are likely dissimilar, using citation relationships as a proxy for similarity.
