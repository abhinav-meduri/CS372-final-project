"""Grid-search style tuning for the PyTorch classifier with simple CV reporting."""
...
def load_features(features_dir: Path = Path("data/features")):
    """Load train/val/test features and names, preferring v2 if present."""
...
def save_results(results, best_config, best_score, test_metrics):
    ...
    with (output_dir / "hyperparameter_results.json").open("w") as f:
...
def main():
    ...
    print(f"Total configurations: {len(all_configs)} (3-fold CV)")
    ...
    for idx, config in enumerate(all_configs):
        ...
        print(f"[{idx+1}/{len(all_configs)}] hidden_dims={hidden_dims}, dropout={dropout}, lr={lr}, wd={wd}, bs={bs}")
        cv_scores = []
        for fold, (train_idx, val_idx) in enumerate(skf.split(X_cv, y_cv)):
            ...
            try:
                ...
                cv_scores.append(val_metrics.get("roc_auc", 0.0))
        ...
        if mean_score > best_score:
            ...
            print("  New best config recorded.")
        print()

    results.sort(key=lambda x: x["mean_cv_score"], reverse=True)
    elapsed_time = time.time() - start_time
    print(f"Finished tuning in {elapsed_time/60:.1f} minutes.")

    if not best_config:
        print("No successful configurations.")
        return

    print("Training best config on train+val and evaluating on test...")
    ...
    save_results(results, best_config, best_score, test_metrics)
